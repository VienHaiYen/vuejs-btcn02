...
export default {
  ...
  watch: {
    topLastestMovies: function () {
      console.log("thi", this.topLastestMovies);
      this.getdata();
    },
  },
  methods: {
    async getdata() {
      console.log("me", this.topLastestMovies);
      this.groupPopularMovie = chunkMaxLength(
        this.topLastestMovies,
        3,
        Math.ceil(this.topLastestMovies.length / 3)
      );
    },
    prev() {
      this.active =
